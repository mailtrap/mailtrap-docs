...
>
> Save my Mailtrap API key in MAILTRAP\_API\_KEY secret.
>
...
>
> My MAILTRAP\_ACCOUNT\_ID env variable (from Supabase Secrets)
>
> Use the same Token I use for the production mode of Mailtrap, as it can also create Contacts.
>
> Pass the name from my form to name field in Mailtrap. Add users to MAILTRAP\_LIST\_ID.
>
...
>
> Save Mailtrap contact\_id (Mailtrap provides it with a response) to mailtrap\_contact\_id column in contacts table.
...
* On form submission:
  * Send an email with its content to MAILTRAP\_MY\_EMAIL secret variable
    * Emails should be sent from form-submissions@demoatmailtrap.com (MAILTRAP\_FROM\_EMAIL), as demoatmailtrap.com is your verified domain in Mailtrap
  * Create a contact in Mailtrap (name, email)
  * Pass the form's "Full name" input value to Mailtrap's custom field name
  * Create a contacts table in Supabase with columns: id (UUID, primary key), name (text), email (text), message (text), created\_at (timestamp), mailtrap\_contact\_id (text)
...
* Email should be sent via Mailtrap:
  * My Mailtrap account is MAILTRAP\_ACCOUNT\_ID (from Supabase Secrets)
  * Link to Mailtrap NodeJS SDK: https://github.com/railsware/mailtrap-nodejs
  * Use it to create contacts and send emails
  * Save Mailtrap API key in MAILTRAP\_API\_KEY secret
