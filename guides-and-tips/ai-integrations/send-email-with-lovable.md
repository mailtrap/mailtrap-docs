...
{% expand title="Landing page prompt for Lovable" %}
Create a SaaS landing page with a contact form that collects the full name, email address, and message.

**Company Information:**
My SaaS is called Acmecomp-deliverability.com, which is about helping customers ensure their emails are delivered without problems. Our form captures leads interested in our deliverability consultation services. We have helped 120+ companies and have case studies to showcase.

**Landing page structure:**
- Hero section describing benefits with button scrolling to form
...
**Configuration:**
Store these in Supabase Secrets:
- MAILTRAP_ACCOUNT_ID
- MAILTRAP_API_KEY
- MAILTRAP-MY-EMAIL
- MAILTRAP-FROM-EMAIL
...
**Form requirements:**
- Add browser validation for email input
- Minimum message length: 50 characters
- Maximum name length: 100 characters

**Form functionality:**
- Send email notifications to MAILTRAP-MY-EMAIL secret variable
- Emails sent from form-submissions@demoatmailtrap.com (MAILTRAP-FROM-EMAIL)
- Create contact in Mailtrap with form data
- Create Supabase contacts table with: id (UUID), name, email, message, created_at, mailtrap_contact_id
- Apply Row-Level Security (RLS) to restrict access
- Display submission confirmation to visitors

**Technical details:**
- Use Mailtrap NodeJS SDK (https://www.npmjs.com/package/mailtrap) latest version
- Documentation: https://github.com/railsware/mailtrap-nodejs
- Email sending example: https://github.com/railsware/mailtrap-nodejs/blob/main/examples/sending/minimal.ts
- Contact creation example: https://github.com/railsware/mailtrap-nodejs/blob/main/examples/contacts/everything.ts
- Write code in TypeScript
- Create atomic files in Supabase Edge Functions for easier debugging

Plan step-by-step, verify results after each step before proceeding.
{% endexpand %}
